...
import model_dir.modules.commons as commons
import utils
from data_utils import TextAudioCollate, TextAudioSpeakerLoader
from model_dir.models import (
    MultiPeriodDiscriminator,
    SynthesizerTrn,
)
from model_dir.modules.losses import discriminator_loss, feature_loss, generator_loss, kl_loss
from model_dir.modules.mel_processing import mel_spectrogram_torch, spec_to_mel_torch
...
def main():
    # Assume CPU-only training on Apple M1 Mac.
    # assert torch.is_available(), "Training is not allowed without PyTorch."
    hps = utils.get_hparams()
    # parser = argparse.ArgumentParser(description='model choice')
    # parser.add_argument('-m', '--model', type=str, default="/Users/jordanharris/Code/PycharmProjects/EZ_RVC/model_dir/G_30400.pth", help='模型路径')
    # parser.add_argument('-c', '--config', type=str, default="/Users/jordanharris/Code/PycharmProjects/EZ_RVC/preprocess/configs/config.json", help='模型路径')
    wandb.init(project="EZ_RVC")
...
def train_and_evaluate(epoch, hps, nets, optims, schedulers, loaders, logger, writers=None):
    ...
    net_g.train()
    net_d.train()
    for batch_idx, items in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
        c, f0, spec, y, spk, lengths, uv, volume = items
        # c, f0, spec, y, spk, lengths, uv, volume = [tensor.to(device=mps_device) for tensor in items]
        # c = c.to(mps_device)
        # f0 = f0.to(mps_device)
        ...
        if global_step % hps.train.log_interval == 0:
            ...
            loss_names = ["loss_disc", "loss_gen", "loss_fm", "loss_mel", "loss_kl"]
