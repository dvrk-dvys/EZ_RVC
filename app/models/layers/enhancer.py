...
from app.models.vocoders.nsf_hifigan.models import load_model
from app.models.vocoders.nsf_hifigan.nvSTFT import STFT
...
class Enhancer:
    ...
    def enhance(
        ...
        if sample_rate == adaptive_sample_rate:
            ...
            if key_str not in self.resample_kernel:
                self.resample_kernel[key_str] = Resample(
                    sample_rate, adaptive_sample_rate, lowpass_filter_width=128
                ).to(self.device)
        ...
        if adaptive_factor != 0:
            ...
            if key_str not in self.resample_kernel:
                self.resample_kernel[key_str] = Resample(
                    adaptive_sample_rate, enhancer_sample_rate, lowpass_filter_width=128
                ).to(self.device)
