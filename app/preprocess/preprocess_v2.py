...
from app.utils.pre_utils import gen_spk_config, convert_to_wav

device = get_device()


class Preprocessor:
    def __init__(self, speaker_name, speech_encoder, target_sample_rate, min_duration=1.0, top_db=30, max_length=10.0, frame_seconds=0.5, hop_seconds=0.1):
        ...
        self.spk_config = gen_spk_config(speaker_name=speaker_name, speech_encoder=speech_encoder)
        ...
        for root, dirs, files in os.walk(input_dir):
            for file in files:
                if file.endswith('.wav'):
                    ...
                    # Call slice method
                    sliced_chunks = self.slice(file_path, file_name, f'data/processed/sliced/')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--raw_audios", type=str, required=True, default=raw_audio_dir, help="Path to raw audio files (directory or single file)")
    parser.add_argument("--speaker_name", type=str, required=True, default=speaker, help="Speaker name identifier (e.g., 'nat_king_cole')")
    parser.add_argument("--speech_encoder", type=str, default="hubertsoft", choices=["hubertsoft", "vec768l12", "vec256l9", "whisper-ppg", "whisper-ppg-large", "cnhubertlarge", "dphubert", "wavlmbase+"], help="Speech encoder type.")
    ...
    Pre.process_raw_audio_dir(args.raw_audios)    # Process raw audio directory (convert to WAV + slice)
